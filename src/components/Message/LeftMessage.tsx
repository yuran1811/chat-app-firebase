...
const LeftMessage: FC<LeftMessageProps> = ({ message, conversation, index, docs, setReplyInfo }) => {
  ...
  return (
    <div id={`message-${message.id}`}>
      <div className={`${conversation.users.length === 2 ? 'px-8' : 'px-[70px]'} -mb-2 flex`}>
        {!!message.replyTo && <ReplyBadge messageId={message.replyTo as string} />}
      </div>
      <div
        onClick={(e) => {
        ...
        {message.type === 'text' ? (
          <>
            {EMOJI_REGEX.test(message.content) ? (
              <div onClick={(e) => e.stopPropagation()} title={formattedDate} className="text-4xl">
                {message.content}
              </div>
            ) : (
              <div
        ...
        {message.type !== 'removed' && (
          <>
            ...
            {isSelectReactionOpened && (
              <ClickAwayListener onClickAway={() => setIsSelectReactionOpened(false)}>
                {(ref) => (
                  <ReactionPopup
                    position={'left'}
                    forwardedRef={ref}
                    ...
                    currentReaction={message.reactions?.[currentUser?.uid as string] || 0}
                  />
                )}
              </ClickAwayListener>
            )}
