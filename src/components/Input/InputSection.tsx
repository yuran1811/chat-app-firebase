...
const InputSection: FC<InputSectionProps> = ({
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    Object.entries(EMOJI_REPLACEMENT).map(([key, value]) => {
      value.forEach((item) => {
        replacedInputValue = replacedInputValue.split(` ${item} `).join(` ${key} `);
      });
    });

    setReplyInfo && setReplyInfo(null);

    addDoc(collection(db, 'conversations', conversationId as string, 'messages'), {
  ...
  const handleReplaceEmoji = (e: any) => {
    if (e.key === ' ') {
      ...
      const lastWord = inputValue.slice(0, e.target.selectionStart).split(' ').slice(-1)[0];
      ...
      Object.entries(EMOJI_REPLACEMENT).map(([key, value]) => {
        value.forEach((item) => {
          if (item === lastWord) {
            const splitted = inputValue.split('');
            splitted.splice(e.target.selectionStart - lastWord.length, lastWord.length, key);
            setInputValue(splitted.join(''));
          }
        });
      });
